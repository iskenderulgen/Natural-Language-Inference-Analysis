...
def fasttext_word_transformer(path, train_loc, dev_loc, shape, transformer_type):
    ...
    if os.path.isfile(path=path + "Processed_SNLI/Fasttext_Processed/train_x.pkl"):
        print("Fasttext_Processed based Pre-Processed train file is found now loading")
        with open(path + 'Processed_SNLI/Fasttext_Processed/train_x.pkl', 'rb') as f:
            train_x = pickle.load(f)
    else:
        print("There is no Fasttext_Processed based pre-processed file of train_X, Pre-Process will start now")
        train_x = create_dataset_ids(nlp=nlp, hypothesis=train_texts1, premises=train_texts2, num_unk=100,
                                     max_length=shape[0])
        with open(path + 'Processed_SNLI/Fasttext_Processed/train_x.pkl', 'wb') as f:
            pickle.dump(train_x, f)

    if os.path.isfile(path=path + "Processed_SNLI/Fasttext_Processed/dev_x.pkl"):
        print("Spacy Fasttext_Processed based Pre-Processed dev file is found now loading")
        with open(path + 'Processed_SNLI/Fasttext_Processed/dev_x.pkl', 'rb') as f:
            dev_x = pickle.load(f)
    else:
        ...
        with open(path + 'Processed_SNLI/Fasttext_Processed/dev_x.pkl', 'wb') as f:
            pickle.dump(dev_x, f)

    if os.path.isfile(path=path + "Processed_SNLI/Fasttext_Processed/fasttext_weights.pkl"):
        print("Spacy Fasttext_Processed weights matrix already extracted, now loading...")
        with open(path + 'Processed_SNLI/Fasttext_Processed/fasttext_weights.pkl', 'rb') as f:
            vectors = pickle.load(f)
    else:
        print("Spacy weight matrix is not found, now extracting...")
        vectors = get_embeddings(vocab=nlp.vocab, nr_unk=100)
        with open(path + 'Processed_SNLI/Fasttext_Processed/fasttext_weights.pkl', 'wb') as f:
